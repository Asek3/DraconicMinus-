    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if (currentRecipe != null) {
            ...
            if (tile.isCrafting.value && tile.craftingStage.value >= 0) {
                ...
                drawCenteredString(fontRendererObj, status + ": " + EnumChatFormatting.GOLD + ((int) (d * 100) + "%"), width / 2, guiTop + 95, state < 1000 ? 0x00FF00 : 0x00FFFF);
            }
...
//            }
    ...
    public void updateScreen() {
        super.updateScreen();
        currentRecipe = RecipeManager.FUSION_REGISTRY.findRecipe(tile, player.worldObj, new BlockPos(tile.xCoord, tile.yCoord, tile.zCoord));
        if (currentRecipe != null) {
            canCraft = currentRecipe.canCraft(tile, player.worldObj, new BlockPos(tile.xCoord, tile.yCoord, tile.zCoord));
        }
    ...
    protected void actionPerformed(GuiButton button) {
    	PacketDispatcher.sendToServer(new PacketTileMessage(tile, (byte) button.id, true, false));
    }
    ...
    private void drawItemStack(ItemStack stack, int x, int y, String altText)
    {
        ...
        this.itemRender.renderItemAndEffectIntoGUI(font, mc.getTextureManager(), stack, x, y);
        this.itemRender.renderItemOverlayIntoGUI(font, mc.getTextureManager(), stack, x, y, altText);
        this.zLevel = 0.0F;
    ...
    public static class EnergyEffect extends GuiEffect {
        ...
        public void renderParticle(float partialTicks) {
            ResourceHelperDP.bindTexture("textures/particle/particles.png");
    ...
    public class StartButton extends GuiButton {
        ...
        public void drawButton(Minecraft mc, int mouseX, int mouseY) {
            ...
            if (visible) {
                ...
                GuiHelper.drawCenteredString(mc.fontRenderer, displayString, xPosition + width / 2, yPosition + (height / 2) - (mc.fontRenderer.FONT_HEIGHT / 2), 0xFFFFFF, false);
            }
        }
